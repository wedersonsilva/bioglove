...
\chapter{Introdução} % ou \input{arquivoexterno}
		...
		Dentre os materiais bioinspirados mais famosos está a tecnologia do Velcro$^{\textregistered}$  que foi inspirada por uma planta chamada Arctium. Essa planta possui uma série de pequenos ganchos que grudam em alguns tecidos. Com isso foi criado o Velcro que é composto de duas partes, os ganchos e as voltas.

		Para flexionar os dedos das mãos, possuímos tendões que passam por dentro de polias. Quando o musculo atrelado à esses tendões de movimenta, obtém-se a flexão ou extensão dos dedos \cite{drricardocirurgiao}. Inspirado por essa biomecânica, foi desenvolvido um prótipo de um sensor de flexão embarcado em uma luva de algodão.

		\section{Estado da arte}

		Existem diversos sistemas que utilizam luvas embarcadas com sensores que conseguem mensurar e processar movimentos através de microcontroladores. Existem produtos comerciais \cite{5DT-ultra}, projetos de baixo custo \cite{simone2004lowcost}, usados em pesquisas médicas \cite{michela2013rehab} , entre outros. 

		As funções atribuídas à essas luvas são variadas. Podem ser usadas para controle de um braço robótico \cite{syed2012armcontroller}, movimentar desenhos tridimencionais em computador \cite{5DT-ultra}, mensurar atividades de pessoas com limitações dos movimentos nos dedos \cite{simone2004lowcost} e até mesmo transcrever gestos em palavras baseado na linguagem americana de sinais \cite{anbarasi2013deafmute}.
