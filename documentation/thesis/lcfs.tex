...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	\chapter{Trabalho Propriamente Dito}
		...
		Como foi demonstrado no movimento de flexão da figura \ref{Fig:hand-tendon-flex1}, através dos tendões, têm-se a movimentação dos dedos na mão. Baseado nessa biomecânica, foi desenvolvido um sistema mecânico semelhante, com o intuito de criar um transdutor de flexão de dedos atrelado a um microcontrolador. 
...
%		\subsection{Flexão e Extensão dos Dedos}
	...
	\end{figure}
	 ...
	 Também foram definidas duas posições principais para o movimento dos dedos. Posição A: Quando todos os dedos estiverem estendidos. Posição B: Quando todos os dedos estiverem flexionados. Posições demonstradas na figura \ref{Fig:glove-flex-and-extend2}.

	 	Para cada posição, o potenciômetro apresenta uma média de valores. Por exemplo, quando o dedo mínimo (1) está extendido (posição A), seu respectivo potenciômetro apresenta valores em torno 157. Quando esse dedo é totalmente flexionado (posição B) os valores ficam em torno de 558. %Então para calcular o deslocamente temos:
...
%	\end{equation}
		...
		O valor digital apresentado pelo potenciômetro é remapeado para uma faixa de valores inteiros que representam posições entre 0 e 9. Quando os valores de todos os potenciômetros são remapeados, eles são concatenados em um único número de 5 digitos, no qual cada dígito representa cada um dos cinco dedos da esquerda para a direita. 
		
		Por exemplo, em uma mensagem "69071" significa que o dedo 1 (mínimo) está na posição 6, o dedo 2 (anelar) está na posição 9 e assim sucessivamente até o dedo 5 (polegar).
...
\end{lstlisting}
		...
		Todos os potenciômetros possuem uma pequena média valores padrão para ambas as posições da luva. Para ler tais valores e futuramente usá-los como referência de cada posição, um pequeno programa foi criado com o propósito de obter essa informação. A variação média do deslocamento entre as posições também foi calculada ($ \Delta Pos = Pos 2 - Pos 1 $). Os resultados obtidos estão resumidos na tabela \ref{Tab:deltapos}.

	\begin{table}[H]
  	\centering
		\caption{Valores para posições 1 e 2 e deslocamento.}
    \begin{tabular}{cccc}
      \midrule
			Número & Pos 1 & Pos 2 & $\Delta$ Pos \\
      \midrule
			1 & 175 & 558 & +383 \\
			2 & 111 & 597 & +486 \\
			3 & 797 & 394 & -433 \\
...
%		Para poder controlar o carrinho, foi necessário criar um protocolo de comunicação que traduzisse os movimentos de flexão dos dedos em comandos e direções para o carrinho que receberia o sinal.
	...
	\begin{figure}[!htb]
	 ...
	 \end{figure}
		...
		\section{Placa Embarcada}
			...
			Com o desenho da placa finalizada no Kicad, foram efetuados os procedimentos para a confecção da PCI em uma placa de fenolite cobreada e sua posterior soldagem de componentes. Chegando assim, ao resultado final mostrado na figura \ref{Fig:phenolic-and-ready}.
	...
	\begin{figure}[!htb]
		 \centering
		 \caption{(a) Placa de fenolite cobreada e (b) resultado final.} 
		 \subfloat[]
		 {
