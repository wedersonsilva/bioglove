...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	\end{figure}
		...
		Para verificar se no sistema criado haveria deslocamento do fio durante a flexão dos dedos, com a mão inicialmente extendida com o dorso voltado para cima, uma das pontas de um fio foi presa na extremidade de um dos dedos. O local da outra ponta do fio foi marcada no dorso mão. Como é demonstrado na figura \ref{Fig:hand-wire-steady-and-flex} (a).
...
%		\subsection{Flexão e Extensão dos Dedos}
		...
		Após o o sistema mecânico ser embarcado na luva, com a mão extendida e mantendo o fio de náilon tensionado, ao flexionar o dedo, o fio é puxado pela ponta do dedo e com isso o cursor do potênciometro é variado. Porém, após a flexionar os dedos, ao realizar o movimento inverso (extenção), não há força que movimente o fio de náilon e nem o cursor do potenciômetro de volta às suas posições iniciais.
...
%		\end{figure}
	...
	\end{table}
		...
		Para possibilitar o controle, conjuntos de posições dos dedos pré definidas indicam movimentos a serem seguidos pelo carrinho.
		
		\subsection{Posições e movimentos}

		Como foi explicado anteriormente, a mensagem transmitida pela luva consiste em conjunto de 5 dígitos no qual cada um deles indica posições de cada dedo. Na mensagem, o primeiro dígito da esquerda indica a posição do dedo 1, que é o dedo mínimo. O segundo dígito indica a posição do dedo 2, ou seja, o dedo anelar. Essa lógica segue até o quinto dígito que representa a posição do dedo polegar.

		5 comandos foram definidos para o carrinho, ir para frente, ir para trás, girar para a esquerda, girar para a direita e parar. Com o intuito de controlar o carrinho através de combinações de flexões e extensões de dedos, 5 conjuntos de posições foram definidas, flexionar os dedos 2 e 3 simultanemanete, flexionar somente o dedo 4, flexionar somente o dedo 5, flexionar o dedo 1 e não flexionar nenhum dedo.
		...
		A biblioteca VirtualWire também deve estar presente no código do microcontrolador embarcado no carrinho. Para receber a mensagem adequadamente, é necessário definir o pino do receptor RF e taxa de transmissão do transmissor RF, também deve-se criar variáveis para guardar a mensagem recebida e o seu tamanho. As mensagens são recebidas \textit{byte} a \textit{byte}.

		Uma função da biblioteca verifica se chegou alguma mensagem, ela indica \textit{true} quando isso acontece. Então para ler a mensagem recebida, é necessário criar um \textit{for} que recebe como parâmetro o tamanho da mensagem recebida. Dentro da função será lida \textit{byte} a \textit{byte} a mensagem enquanto a função \textit{for} for verdadeira.
...
\end{lstlisting}
		...
		Usando o pseudocódigo apresentado e sabendo que na mensagem recebida o primeiro dígito indica a posição atual do dedo 1, então a função for irá rodar pelo menos 5 vezes, armazenando em cada posição do vetor \texttt{byte\_recebido[]} as informações de cada dedo. Dessa maneira, a posição recebida do dedo 1 estará em \texttt{byte\_recebido[0]}, enquanto que a do dedo 2 está em \texttt{byte\_recebido[1]}. Sendo assim até o dedo 5 que estará em \texttt{byte\_recebido[4]}.
		
		Cinco funções foram criadas para comandar a ponte H e dessa forma movimentar os motores do carrinho, FRENTE( ), TRAS( ), ESQUERDA( ), DIREITA( ) e PARA( ). Ao chamar qualquer uma dessas funções, o carrinho passa a se movimentar na direção e sentido definidas.
...
%		Para poder controlar o carrinho, foi necessário criar um protocolo de comunicação que traduzisse os movimentos de flexão dos dedos em comandos e direções para o carrinho que receberia o sinal.
	...
	\end{figure}
			...
			Após a conclusão do desenho da placa, foram efetuados os procedimentos para a confecção desta PCI em uma placa de fenolite cobreada, além de sua posterior soldagem de pinos e componentes. A figura \ref{Fig:phenolic-and-ready} mostra os resultados após o procedimento descrito. Já a figura \ref{Fig:glove-ready1} mostra o resultado final após juntar a luva de algodão e a placa de circuito impresso recém concluída.
