...
%		A biologia inspira diversos projetos de engenharia. Na área da robótica, por exemplo, existe um robô em formato de salamandra que é capaz de se movimentar bem entre a água e a terra, há um pequeno robô inspirado em moscas que, devido ao seu formato, possui excelente decolagem vertical na hora do vôo. Existe ainda um robô que consegue escalar paredes através de um material adesivo que foi inspirado pela lagartixa \cite{pfeifer2007biorobot}. 
		...
		A mão exerce uma importante função na interação do ser humano com a tecnologia. Muitos periféricos necessitam da mão para controlá-los, é o caso do mouse e teclado de computador, do controle remoto da televisão e do videogame e também dos famosos tablets e smartphones.	Entretando, para aplicações de realidade virtual a imersão se torna ainda melhor, quando são usados movimentos naturais de controle. E para isso o uso de uma luva de controle pode deixar a interação ainda mais real para o usuário. 
	
		Luvas embarcadas com sensores são utilizadas não só em aplicações de realidade virtual, podem ser usadas para controle de um braço robótico \cite{syed2012armcontroller}, movimentar desenhos tridimencionais em computador \cite{5DT-ultra}, mensurar atividades de pessoas com limitações dos movimentos nos dedos \cite{simone2004lowcost} e até mesmo transcrever gestos em palavras baseado na linguagem americana de sinais \cite{anbarasi2013deafmute}. Existem produtos comerciais \cite{5DT-ultra}, projetos de baixo custo \cite{simone2004lowcost}, usados em pesquisas médicas \cite{michela2013rehab}, entre outros. 

		Dentre os materiais bioinspirados mais famosos está a tecnologia do Velcro$^{\textregistered}$ que foi inspirada por uma planta chamada Arctium. Essa planta possui uma série de pequenos ganchos que grudam em alguns tecidos. Por conta desse comportamento, o Velcro é composto de duas partes, os ganchos e as voltas.

		Para flexionar os dedos das mãos, possuímos tendões que passam por dentro de polias. Quando o músculo atrelado à esses tendões de movimenta, obtém-se a flexão ou extensão dos dedos \cite{drricardocirurgiao}. Inspirado por essa biomecânica, foi desenvolvido um prótipo de um sensor de flexão embarcado em uma luva de algodão.
		...
		O capítulo 2 falará sobre a mecânica que inspirou a construção do sensor, sobre como funciona a aquisição dos movimentos, explica o funcionamento do potenciômetro que é a peça chave do sensor, e também menciona as características do microcontrolador e transmissor utilizados.
		
		O capítulo 3 explica mais detalhadamente sobre o funcionamento do sensor, adaptação do sensor na luva, também explica o algoritmo de programação do software que trata o sinal recebido e os ajustes que foram necessários, além de falar também sobre o protocolo de comunicação criado. E por último explica como todo o sistema foi construído e montado.

		O capítulo 4 apresentará os resultados e análises dos testes realizados na BioGlove em comparação à outras luvas desenvolvidas.

		Por fim, o capítulo 5 mostrará a conclusão do trabalho.
...
%		No sensor de flexão bioinspirado desenvolvido, para comparar com o trabalho de \cite{anbarasi2013deafmute}, com a luva vestida, foram obtidos os valores digitais captados nas duas posições da mão demostradas na figura \ref{Fig:hand-flexsensor-degrees1}.
    ...
    \fonte{Adaptado de \cite{anbarasi2013deafmute} e produzido pelo autor.}
		...
		Dessa forma é possível afirmar que, neste teste, o sensor bioinspirado conseguiu desempenho equiparado a um sensor comercial. Sendo que um sensor flex comercial possui um custo relativamente superior. Portanto, em projetos de baixo custo, respeitando suas limitações e requisitos do projeto, seria viável usar o sistema bioinspirado desenvolvido.
			...
			O trabalho de \cite{michela2013rehab} busca, através de uma luva sensorizada, mensurar a flexão dos dedos para estudos de reabilitação. A luva é embarcada sensores flex comerciais, um microcontrolador e um transmissor de rádio frequência. Toda a eletrônica é alimentada por uma bateria em formato de botão com tensão nominal de 3V e capacidade de 1000mAh. Obteve desempenho aproximado de uma semana.
...
%Controle de um carrinho		
    ...
    \begin{tabular}{c|c|c|c}
      ...
      \midrule                                            					
			...
			BioGlove						& 1 LiPo 7,4V			& 300 mAh			&	10 horas\\	
      \midrule
    \end{tabular}
	...
	% ----------------------------------------
		...
		\begin{itemize}
			...
			\item Instalar antenas melhores em ambos os módulos de rádio frequência.
