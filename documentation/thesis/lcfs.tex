...
\begin{lstlisting}	
	...
	// Concatena\textit{string} + inteiros para obter uma unica string
	mensagem =\textit{string} + dedo1 + dedo2 + dedo3 + dedo4 + dedo5;

	// Converte a\textit{string} em um vetor de char
...
\chapter{Resultados e análises}
    ...
    \fonte{Adaptado de \cite{anbarasi2013deafmute} e produzido pelo autor.}
		...
		Com base no desempenho geral apresentado pela Tabela \ref{Tab:NPos0-90}, verifica-se que usando o sensor bioinspirado que compõe a BioGlove, os sensores dos dedos anelar e médio apresentaram os melhores desempenhos mesmo quando comparados ao sensor flex comercial de 4,5 polegadas. Além do mais, 3 dos 4 sensores da BioGlove possuíram desempenho superior se comparados apenas ao sensor flex comercial de 2,5 polegadas. Dessa forma é possível afirmar que os sensores bioinspirados da BioGlove conseguiram desempenho equiparado ao de um sensor comercial. Sendo que um sensor flex comercial possui um custo relativamente superior.
			...
			Mobilidade é um dos requisitos do projeto da BioGlove. Por conta desse fato, foi decidido que a alimentação dos componentes eletrônicos da luva seria feita através de uma pequena bateria LiPo. Outros sistemas envolvendo luvas sensorizadas móveis também costumam usam baterias ou até mesmo pilhas, sendo que o desempenho de cada projeto neste quesito pode variar de acordo com o \textit{hardware} implementado e a capacidade da fonte de alimentação escolhida. Nesta seção, apresentan-se testes de desempenho da bateria da Bioglove que foram comparados com outros três trabalhos relacionados com luvas sensorizadas, sendo um deles um produto comercial.
