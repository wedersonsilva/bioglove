...
%		\subsection{Flexão e Extensão dos Dedos}
	...
	\end{figure}
		...
		Na luva, além do transdutor, há um microcontrolador que capta os movimentos do potenciômetro. O sinal chega em portas analógicas de um Arduino modelo Nano que converte a variação de tensão do potenciômetro em valores entre 0 e 1023. 
	 ...
	 Também foram definidas duas posições principais para o movimento dos dedos. Posição 1: Quando todos os dedos estiverem estendidos. Posição 2: Quando todos os dedos estiverem flexionados. Posições demonstradas na figura \ref{Fig:glove-flex-and-extend2}.

	 Cada potenciômetro apresenta uma média de valores para cada posição. Por exemplo, quando o dedo mínimo (1) está extendido (posição 1), apresenta valores em torno 157. Quando esse dedo é totalemnte flexionado (posição 2) o valor fica em torno de 558. Então para calcular o deslocamente temos:
